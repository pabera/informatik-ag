...
# Spielvariablen
punktzahl = 0
passierte_roehren = []  # Um die passierten Roehren zu verfolgen
...
def pruefe_kollision(roehren):
    vogel_rechteck = pygame.Rect(vogel_x - vogel_radius, vogel_y - vogel_radius, vogel_radius * 2, vogel_radius * 2)
    for roehre in roehren:
        if vogel_rechteck.colliderect(roehre):
            return True
    if vogel_y + vogel_radius > HOEHE or vogel_y - vogel_radius < 0:
        return True
    return False
...
while laeuft:
    ...
    # Kollisionen pruefen
    if pruefe_kollision(roehren_liste):
        laeuft = False

    # Punktzahl aktualisieren
    for i in range(0, len(roehren_liste), 2):
        roehre = roehren_liste[i]
        if roehre.right < vogel_x and roehre not in passierte_roehren:
            punktzahl += 1
            passierte_roehren.append(roehre)
    ...
    punktzahl_text = schriftart.render(f"Punkte: {punktzahl}", True, SCHWARZ)
    bildschirm.blit(punktzahl_text, (10, 10))
